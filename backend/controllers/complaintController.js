...
exports.postComplaint = async (req, res) => {
  try {
    ...
    if (!username || !pnr || !description || !issueDomain) {
      ...
      return res.redirect(
        "/complaint?error=true&message=All fields are required"
      );
    }
    ...
    if (req.file)
      await fs
        ...
        .catch((err) => console.error("Error deleting temp file:", err));
    res.redirect(
      `/complaint?error=true&message=${encodeURIComponent(error.message)}`
    );
  }
};
...
exports.deleteComplaint = async (req, res) => {
  try {
    ...
    if (!currentUser || complaint.username !== currentUser) {
      return res
        .status(403)
        .json({ error: "Unauthorized to delete this complaint" });
    }
  ...
  try {
    ...
    if (!username || !file) {
      if (file)
        await fs
          ...
          .catch((err) => console.error("Error deleting temp file:", err));
      return res.redirect(
        "/complaint/upload-image?error=true&message=Missing username or image"
      );
    }
    ...
    if (req.file)
      await fs
        ...
        .catch((err) => console.error("Error deleting temp file:", err));
    res.redirect(
      `/complaint/upload-image?error=true&message=${encodeURIComponent(
        error.message
      )}`
    );
  }
